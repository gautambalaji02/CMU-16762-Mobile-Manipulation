...
class MoveMe(HelloNode):
    def __init__(self):
        ...
        # Stow joint positions (read once at start)
        stow_lift = self.get_joint_pos('joint_lift')
        stow_arm_l3 = self.get_joint_pos('joint_arm_l3')
        stow_arm_l2 = self.get_joint_pos('joint_arm_l2')
        stow_arm_l1 = self.get_joint_pos('joint_arm_l1')
        stow_arm_l0 = self.get_joint_pos('joint_arm_l0')
        stow_yaw = self.get_joint_pos('joint_wrist_yaw')
        stow_pitch = self.get_joint_pos('joint_wrist_pitch')
        stow_roll = self.get_joint_pos('joint_wrist_roll')

        # Define the 4 goal states
        goals = [
            
            #Move forward 0.2m, left 0.2m, rotate pi/2 CCW; lift arm to 0.5m
            (0.2, 0.2, np.pi / 2,
             0.5, stow_arm_l3, stow_arm_l2, stow_arm_l1, stow_arm_l0,
             stow_yaw, stow_pitch, stow_roll),

            #Move forward 0.2m , rotate pi/2 CCW; extend arm to 0.4m
            (0.2, 0.0, np.pi / 2,
             0.5, 0.1, 0.1, 0.1, 0.1,
             stow_yaw, stow_pitch, stow_roll),

            #Move forward 0.2m, rotate pi/2 CCW; rotate wrist 45° on each axis
            (0.2, 0.0, np.pi / 2,
             0.5, 0.1, 0.1, 0.1, 0.1,
             stow_yaw + np.pi / 4, stow_pitch + np.pi / 4, stow_roll + np.pi / 4),

            #Move forward 0.2m , rotate pi/2 CCW; stow the arm
            (0.2, 0.0, np.pi / 2,
             stow_lift, stow_arm_l3, stow_arm_l2, stow_arm_l1, stow_arm_l0,
             stow_yaw, stow_pitch, stow_roll),
        ]

        for i, goal_joints in enumerate(goals):
            print(f'--- Planning Step {i}  ---')
            goal_state = RobotState(moveit.get_robot_model())
            goal_state.set_joint_group_positions(planning_group, list(goal_joints))

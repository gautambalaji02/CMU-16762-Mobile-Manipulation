...
# Creating a ROS2 node compatible with to communicate with the stretch driver node
class StretchIKNode(hm.HelloNode):
    def __init__(self):
        hm.HelloNode.__init__()
        hm.HelloNode.main(self, 'stretch_ik_node', 'stretch_ik_node', wait_for_first_pointcloud=False)
        self.get_logger().info("Stretch IK Node has started.")

    # Function to home, calibrate and stow the robot in the physical world
    def prepare(self):
        self.home_the_robot()
        self.get_logger().info("Robot is now calibrated and homed")
        self.stow_the_robot()
        self.get_logger().info("Robot stowed and ready for IK commands.")
...
# Modifying the urdf to add a revolute joint
...
##  Adding virtual base rotation joint
joint_base_revolute = urdfpy.Joint(name= 'joint_base_rotation',
                                   parent='base_link',
                                   child='link_base_rotation',
                                   joint_type='revolute',
                                   axis=np.array([0.0, 0.0, 1.0]),
                                    origin=np.eye(4, dtype=np.float64))
modified_urdf._joints.append(joint_base_revolute)

# Add a virtual base rotation link
link_base_rotation = urdfpy.Link(name='link_base_rotation',
                                    inertial=None,
                                    visuals=None,
                                    collisions=None)
modified_urdf._links.append(link_base_rotation)
# Add virtual base prismatic joint and link

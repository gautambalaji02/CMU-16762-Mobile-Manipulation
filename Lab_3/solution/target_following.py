...
class IKTargetFollowing(HelloNode):
    ...
    def get_goal_pose_in_base_frame(self, goal_msg):
        # TODO: ------------- start --------------
        # look up the transform from the goal's camera frame to base_link
        transform = self.tf_buffer.lookup_transform(
            self.target_frame,          # target: base_link
            goal_msg.header.frame_id,   # source: gripper_camera_color_optical_frame
            rclpy.time.Time()           # use the latest available transform
        )
        # apply the transform to the PoseStamped message
    ...
    def get_gripper_pose_in_base_frame(self):
        # TODO: ------------- start --------------
        # look up where link_grasp_center currently is relative to base_link
        gripper_transformed = self.tf_buffer.lookup_transform(
            self.target_frame,      # target: base_link
            self.gripper_frame,     # source: link_grasp_center
            rclpy.time.Time()       # use the latest available transform
        )
        # TODO: -------------- end ---------------
    ...
    def compute_waypoint_to_goal(self, goal_pos, gripper_pos):
        ...
        if dist > self.delta:
            # goal is too far to reach in one step — move delta toward it
            direction = (goal_pos - gripper_pos) / dist
            waypoint_pos = gripper_pos + self.delta * direction
        else:
            # goal is close enough to reach directly
    ...
    def main(self):
        ...
        # TODO: ------------- start --------------
        # create TF2 buffer and listener so we can look up coordinate frame transforms
        self.tf_buffer = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)

        # subscribe to the goal pose published by the object detector node
